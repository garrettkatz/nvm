...
if __name__ == '__main__':
    ...
    run_trial_fun = run_kv_trial
    # run_trial_fun = run_array_trial

    num_trials = 5
    max_passive_ticks=1000
    ...
    for n in range(num_trials):
        ...
        kvn = bmn.make_tanh_kvn(layer_sizes = [N]*3)
        trial_kwargs.append({
            # 'mnh': MemoryNetHarness(MockMemoryNet(N, noise=0.00), max_passive_ticks=max_passive_ticks, seed=seed),
            'mnh': MemoryNetHarness(kvn, max_passive_ticks=max_passive_ticks, seed=seed),
            'keys': keys,
            'values': values,
        })
