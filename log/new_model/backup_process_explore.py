...
def build_model(samples):
    ...
    for s in samples:
        ...
        a = np.array(matrix)
        estimate = np.linalg.solve(a, log_targets)
        sugg = [exp(e) for e in estimate]
    ...
    for i in range(3):
        ...
        plt.title("Parameter Change Histogram %d" % i)
        plt.hist(data, 100)
...
def validate(samples, coeff_means, sugg_mean):
    ...
    for i,label in enumerate(["Latency", "Amybdala BOLD", "vmPFC BOLD"]):
        ...
        print("%30s: mean: %9.2f | error mean: %9.7f | error std: %9.7f | errstd/mean: %9.7f | sign errors: %7.5f" % (
            label,
            means[i],
            err_mean,
            err_std,
...
samples = load_data("explore", num_faces, lat_min, lat_max)
...
coeff_means, sugg_mean = build_model(training)
validate(test, coeff_means, sugg_mean)
